  ...
  function Card({item}) {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          props.navigation.navigate('Cards', {cards: content});
        }}
        activeOpacity={0.8}
        onLongPress={() => openDeleteDialog(cartigory)}>
        <View style={styles.imageConatiner}>
          {imageSrc ? (
            <Image source={{uri: imageSrc}} style={styles.image} />
          ) : (
            <Image
              source={require('../../assets/icons/image_1.jpg')}
              style={styles.image}
            />
          )}
        </View>
        <View>
          ...
          <View>
            <Text style={styles.cartigory}>{`@${cartigory}`}</Text>
          </View>
        </View>
      </TouchableOpacity>
    );
